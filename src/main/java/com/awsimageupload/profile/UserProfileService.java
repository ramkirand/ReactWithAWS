...
public class UserProfileService {
	...
	public void uploadUserProfileImage(String userProfileId, MultipartFile file) {
		...
		String path = BucketName.BUCKETNAME.getBucketName() + "/images";
	...
	public byte[] downloadUserProfileImage(String userProfileId) {
		...
		String path = String.format("%s/%s", BucketName.BUCKETNAME.getBucketName(), user.getUserProfileId());
	...
	private void isDocument(MultipartFile file) {
		if (!Arrays.asList(WILDCARD.getMimeType()).contains(file.getContentType())) {
			throw new IllegalStateException("File must be an document [" + file.getContentType() + "]");
		}
	}
